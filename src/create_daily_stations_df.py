...
def str_to_int(arr):
    return map(lambda x: int(remove_last_char(x)), arr)

def identity(arr):
    return arr
def daily_stations_df():
    max_temps = format_df('../data/dly-tmax-normal.csv', 'max_temps', identity)
    # print(max_temps[100:110])
    min_temps = format_df('../data/dly-tmin-normal.csv', 'min_temps', identity)
    gdd_40 = format_df('../data/dly-grdd-base40.csv', 'gdd_40', identity)
    gdd_50 = format_df('../data/dly-grdd-base50.csv', 'gdd_50', identity)
    mtd_precip = format_df('../data/mtd-prcp-normal.csv', 'mtd_precip', identity)
    mtd_snow = format_df('../data/mtd-snow-normal.csv', 'mtd_snow', identity)
    ytd_precip = format_df('../data/ytd-prcp-normal.csv', 'ytd_precip', identity)
    ytd_snow = format_df('../data/ytd-snow-normal.csv', 'ytd_snow', identity)
    dly_precip_50 = format_df('../data/dly-prcp-50pctl.csv', 'dly_precip_50', identity)
    dly_precip_75 = format_df('../data/dly-prcp-75pctl.csv', 'dly_precip_75', identity)
